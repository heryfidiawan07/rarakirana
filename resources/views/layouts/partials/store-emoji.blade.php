<div class="emojiforum">
	@foreach($emojis as $emo)
		<div class="emoloop">
			@if(Auth::check())
				@if(Auth::user())
					<a href="/store-like/{{$emo->id}}/store/{{$store->id}}">
			...
			<span style="margin-left: 5px;">{{$store->likes->where('emoticon_id',$emo->id)->count()}}</span>
			<span data-mid="{{$emo->id}}" data-pid="{{$emo->id}}" type="button" data-toggle="modal" data-target="#usersvote_{{$emo->id}}" class="caret caretVotes" data-url="/emoji/{{$emo->id}}/get/{{$store->id}}/store-users-vote"></span>
