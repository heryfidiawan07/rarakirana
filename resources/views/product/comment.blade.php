@if($product->comment_status == 1)
  ...
  <div class="row">
    <div class="col-xs-12">
      ...
      @if(Auth::check())
        @if(Auth::user())
          <form action="/comment/{{$product->id}}/product" method="POST">
            {{csrf_field()}}
            <div class="form-group{{ $errors->has('description') ? ' has-error' : '' }}">
            ...
            <div class="form-group">
              <button type="submit" class="btn btn-primary btn-sm" id="newcomment" prod-id="{{$product->id}}" com-url="/comment/{{$product->id}}/product">
                <span class="glyphicon glyphicon-send" aria-hidden="true"></span> kirim
              </button>
            </div>
          </form>
